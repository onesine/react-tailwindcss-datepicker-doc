    ...
    return (
        <Menu as="div" className="relative">
            <Menu.Button className="hidden md:block">
                {theme === "light" && <SunIcon className="w-6 h-6 text-sky-400" />}

                {theme === "dark" && <MoonIcon className="w-6 h-6 text-sky-400" />}
                ...
                <Menu.Items className="absolute right-0 mt-2 w-32 py-1 origin-top-right rounded-md bg-white dark:bg-slate-800 dark:border-slate-700 border shadow-lg focus:outline-none">
                    {["Light", "Dark", "System"].map((item, index) => (
                        <Menu.Item key={index}>
                            <button
                                className={
                                    "w-full flex items-center space-x-3 font-semibold hover:bg-gray-50 dark:hover:bg-white/10 px-4 py-1"
                                }
                                onClick={() => changeTheme(item.toLowerCase() as ThemeType)}
                            >
                                {item === "Light" && (
                                    <SunIcon
                                        className={`w-6 h-6 ${item.toLowerCase() === theme ? "text-sky-500" : "text-gray-400"}`}
                                    />
                                )}
                                ...
                                <span
                                    className={`${item.toLowerCase() === theme ? "text-sky-600" : "text-gray-600 dark:text-white/80"}`}
                                >
                                    {item}
                                </span>
                            </button>
